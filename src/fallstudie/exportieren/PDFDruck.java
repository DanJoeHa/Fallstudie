...
/**
 * @author Angelos, Philipp Klasse zum Exportieren von PDF Dateien
 */
public class PDFDruck {
	...
	/**
	 * Methode zum Zeichnen der fertigen Tabelle in PDF
	 */
	public static void drawTablePDF(PDPage page,
			PDPageContentStream contentStream, float y, float margin,
			String[][] content) throws IOException {
		final int rows = content.length;
		final int cols = content[0].length;
		final float rowHeight = 20f;
		final float tableWidth = page.getMediaBox().getWidth() - margin
				- margin;
	...
	/**
	 * Methode zum erzeugen der Tabelle für das PDF. 
	 */
	public static void generateTablePDF(String[][] TabellenContent,
			String PDFUeberschrift, String[] ueberschriften)
			throws IOException, COSVisitorException, IllegalArgumentException,
			PrinterException {
		...
		do {
			...
			if (zahler != 0) {
				...
				PDFDruck.drawTablePDF(page, contentStream, 720, 10,
						komplettArray);
				// Überschrift
				contentStream.beginText();
		...
		File dir = new File(System.getProperty("user.home")
				+ "\\Desktop\\PDF aus Stippler");
		if (!dir.exists())
			dir.mkdir();

		doc.save(System.getProperty("user.home")
				+ "\\Desktop\\PDF aus Stippler\\PDFExport_"
